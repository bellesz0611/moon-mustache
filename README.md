...
- current git history: `27` commits
- key demos: CLI rendering, template scanning, bundle generation, scenario report, downstream consumer, Vue playground
...
- 手写 MoonBit 代码约 `5661` 行
- 导入上游 fixture 后生成的 MoonBit 测试资产约 `1513` 行
- 合计约 `7174` 行 MoonBit 代码
...
- 64 个自动化测试
- 覆盖 `fmt / info / check / test / showcase / scaffold_demo / spec_report / official_spec_report / scenario_report / downstream_consumer / benchmarks / cli-file-flow / cli-scan / bundle-check-only` 的 CI
