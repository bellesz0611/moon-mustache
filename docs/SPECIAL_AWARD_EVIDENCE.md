...
Moon Mustache 是 MoonBit 原生 Mustache 引擎与安全多文件生成器。它不是只验证几个示例，而是用五类互补证据回答五种风险：

| 风险 | 证据 | 复现入口 |
| --- | --- | --- |
| 核心逻辑回退 | MoonBit 单元与回归测试 | `moon test --deny-warn --target wasm-gc` |
| 规范理解偏差 | 上游 `mustache/spec` fixture | `moon run official_spec_report` |
...
CI 会在 `wasm`、`wasm-gc`、`js`、`native` 四个目标执行 check/build/test；`js` lane 额外运行 CLI 黑盒集成套件并上传 JSON 结果，`wasm-gc` lane 上传覆盖率摘要和 Cobertura XML。
...
- 官方 fixture 验证对 Mustache 既有语义的理解，不把自定义用例当作规范本身。
