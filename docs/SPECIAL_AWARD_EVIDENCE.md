...
Moon Mustache 是 MoonBit 原生 Mustache 引擎与安全多文件生成器。它不是只验证几个示例，而是用七类互补证据回答七种风险：
...
| 组合输入遗漏 | 四个固定 seed 的 `mustache.js` 差分测试，失败自动最小化 | `cd playground && npm run differential` |
| CLI 只“能跑”但契约错误 | 真实子进程、退出码和文件产物断言 | `python scripts/test_cli_integration.py` |
| 后端行为发生分叉 | 同一 golden 输出与诊断 corpus 跨 `wasm`、`wasm-gc`、`js`、`native` 逐字节比较 | `python scripts/test_backend_conformance.py`，CI 强制包含 native |
| 未测试代码持续增长 | 核心覆盖率 CI 门禁和 Cobertura artifact | GitHub Actions `ci.yml` |
| 测试虽通过但无法识别关键回退 | 覆盖语义、安全路径和四种资源边界的可控故障矩阵，要求对应回归测试真实失败 | `python scripts/run_fault_injection.py` |
...
- 差分测试以四个固定 seed 生成组合输入，对比另一个成熟实现，并为失败样本生成精确重放命令和有界自动最小化结果。
- CLI 集成测试从 Python 启动真实 MoonBit JS CLI，断言 stdout、退出码和磁盘产物。
- 四后端矩阵负责发现目标相关问题；golden conformance 进一步把同一 corpus 的输出与诊断直接比较，Windows job 负责浏览器桥和路径差异。
...
[`scripts/run_fault_injection.py`](../scripts/run_fault_injection.py) 每次只在临时项目副本中注入一个故障，并运行一个直接对应风险的回归测试。矩阵覆盖转义、truthiness、路径穿越、父上下文，以及输出长度、Section 迭代、总步骤和 Partial 深度四种资源边界。所有 mutant 必须被测试失败杀死；编译错误、目标锚点漂移、超时和存活 mutant 都会让命令失败，并同时输出 JSON 与逐 mutant JUnit。具体故障和边界见 [Controlled Fault Injection](FAULT_INJECTION.md)。
